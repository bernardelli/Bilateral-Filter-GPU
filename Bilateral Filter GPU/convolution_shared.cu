...
{	
	...
	s_image[s_ix + s_iy*s_dim_x] = input[cube_idx];
...
#pragma unroll
	for (int i = 0; i < kernel_size; i++)
	{
		result += kernel[i] * s_image[s_ix + (s_iy - i + radius_size)*s_dim_x];
	}
...
void callingConvolution_shared(float *dev_cube_wi_out, float *dev_cube_w_out, float *dev_cube_wi, float *dev_cube_w, const float *dev_kernel_xy, int kernel_xy_size, const float *dev_kernel_eps, int kernel_eps_size, dim3  image_dimensions)
{
	...
	*/
	...
	shared_memory_size = block_dim_y*(block_dim_x + 2 * k_radius_xy);
	...
	shared_memory_size = block_dim_eps*(block_dim_x + 2 * k_radius_eps);


	const dim3 block3(block_dim_eps, block_dim_y); //threads per block 32 32
